...
| `Project_User` | Roles: Manager / Executive only (absent or inactive ⇒ internal `Requester` — not an approver, but can still create change requests). `ProjectSponsor` is not a role here — picked directly from `Employee List` |
...
```
(Create only) Pending Sponsor ──Sponsor assigns Manager──▶ Pending Manager ──approve──▶ Pending Executive ──approve──▶ APPLY to Project_List ─▶ Approved
                                                                  │ reject (Remark ⚠)            │ reject (Remark ⚠)      │ apply fails
                                                                  ▼                              ▼                        ▼
                                                               Rejected                       Rejected            stays Pending Executive (retry)
```

Update/Delete CRs skip `Pending Sponsor` entirely and start at `Pending Manager` — the target project already has a `ProjectManager` from its own Create flow.
...
Anyone with an email in the tenant can create change requests, including Managers and Executives — there is no separate "Requester" role in `Project_User`; it is only the internal default `gUserRole` for anyone with no active `Project_User` row. **`ProjectSponsor` is not a `Project_User` role either** — any employee can be picked as a project's Sponsor directly from `Employee List` on `CreateProjectScreen`, so "Sponsor" below is a per-CR relationship (like `RequestedBy`), not a `gUserRole` value.
...
| (any employee) as Sponsor | + "Assign Manager" tab filtered `Pending Sponsor` **AND** assigned as that CR's `ProjectSponsor` (acts as Step 0, Create only — picks the `ProjectManager` and submits). Not a `Project_User` role — see above |
...
**Approval authorization is per-project, not just per-role.** A Sponsor only sees/can act on a CR if they are that CR's `ProjectSponsor`; a Manager only if they are that project's `ProjectManager`; an Executive only if they are that project's `ProjectOwner`. It is not a shared "any Manager can approve any pending-Manager CR" queue. The responsible person is resolved as:
- **Create CR**: `ProjectSponsor`/`ProjectManager`/`ProjectOwner` proposed directly on the CR (`gSelectedCR.ProjectSponsor`/`.ProjectManager`/`.ProjectOwner`) — the project doesn't exist yet, so there's nothing else to check against. `ProjectManager` is blank until the Sponsor assigns it (Step 0).
- **Update/Delete CR**: the *live* target project's `ProjectManager`/`ProjectOwner` (`gLiveTarget.ProjectManager`/`.ProjectOwner`, looked up via `TargetItemID`) — the CR itself doesn't carry these fields, and there's no `ProjectSponsor`/Step 0 for these request types at all.
...
This authorization check is duplicated in 3 places per role (Manager/Executive) and must stay in sync if the rule ever changes: the `galChangeRequests` gallery `Items` filter (which CRs appear in "To Approve"), `conActionBar`'s `Visible` (whether Approve/Reject buttons show for the selected CR), and `gApprovalsPendingCount` (the badge count, in both `App.OnStart` and `AllProjectsScreen.OnVisible`). Sponsor's Step 0 duplicates the same check in the equivalent 3 places: the `galChangeRequests` "Assign Manager" case (`gApprovalsTab = "Assign Manager"` in the `Items` `Switch`), `rowAssignManager`'s `Visible`, and `gApprovalsPendingCount`'s `Pending Sponsor` branch — but note that branch checks `ProjectSponsor.Id = gCurrentEmployee.ID` **without** a `gUserRole` guard, unlike the Manager/Executive branches, since `ProjectSponsor` isn't gated by a `Project_User` role (see below). `cmbOwner` on `CreateProjectScreen` only offers Executives as choices, and `cmbAssignManager` on `ApprovalsScreen`'s "Assign Manager" tab only offers Managers (see `docs/sharepoint-schema.md`), so `ProjectOwner`/`ProjectManager` are guaranteed to hold someone with the matching role — this authorization model depends on that constraint holding. **`ProjectSponsor` is the one exception**: `cmbSponsor` on `CreateProjectScreen` sources directly from `Employee List`, not from a `Project_User`-scoped role, so it deliberately does *not* hold that guarantee — any employee can be picked, which is also why the "Assign Manager" tab and the "Approvals (n)" button on `AllProjectsScreen` are visible to everyone (`gIsApprover || gApprovalsPendingCount > 0`), not gated on a role.

Email notifications follow the same per-project targeting, not a role broadcast: `SubmittedToSponsor` goes to the CR's own `ProjectSponsor` (Create only), `SubmittedToManager` goes to the CR's/project's specific `ProjectManager` (`Coalesce(LookUp('Employee List', ID = <ProjectManager.Id>).Email, "app.admin@maxbiocare.com")` — fired either on Update/Delete submit or once the Sponsor finishes assigning a Manager), and `ManagerApprovedToExecutive` goes to the specific `ProjectOwner` the same way.
...
2. Seed `Project_User` with at least one account per role (Manager/Executive). No seeding needed for Sponsor — any employee in `Employee List` can be picked.
...
- **Everyone** (not just Manager/Executive) has raw `Edit Items` on `Project_ChangeRequests` — broadened on purpose so any employee picked as `ProjectSponsor` can Patch the "Assign Manager" step (see `docs/sharepoint-schema.md` §SharePoint list permissions, "Accepted trade-off"); Executive additionally has `Edit Items` on `Project_List`. Anyone with SharePoint UI access could in principle bypass the app's guarded formulas by editing those lists directly. Not closable with list permissions alone; would need moving the guarded Patches into a Power Automate flow with a locked-down connection.
