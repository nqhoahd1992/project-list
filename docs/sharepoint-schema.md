...
- **Exception:** `Department` on `Project_List` / `Project_ChangeRequests` is plain **Text** (not Choice) — Patch the raw string, no `{Value:}` wrapper. Its picker options are sourced live from `Distinct('Employee List', department.Value)` rather than a fixed Choice value set.
...
| `Department` | Text | Single department name; picker options come from `Distinct('Employee List', department.Value)`, not a Choice column — see `.department` on Employee List |
...
| `department` | Choice | internal name is **lowercase**; source of the `Department` picker options on `Project_List`/`Project_ChangeRequests` via `Distinct('Employee List', department.Value)` |
