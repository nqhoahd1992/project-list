...
- **Exception:** `Department` on `Project_List` / `Project_ChangeRequests` is plain **Text** (not Choice) — Patch the raw string, no `{Value:}` wrapper. Its picker options are sourced live from `Distinct(Filter('Employee List', !IsBlank(department.Value)), department.Value)` rather than a fixed Choice value set — the `Filter` drops employees with no department set, so no blank option appears in the picker.
...
| `Department` | Text | Single department name; picker options come from `Distinct(Filter('Employee List', !IsBlank(department.Value)), department.Value)`, not a Choice column — see `.department` on Employee List |
| `ProjectOwner` | Lookup→Employee List | Business owner, `{Id, Value}` (→Title). Picker on `CreateProjectScreen` is scoped to active `Project_User` accounts with `Role.Value = "Executive"` (`ForAll(Filter(Project_User, IsActive && Role.Value = "Executive"), LookUp('Employee List', ID = EmployeeID.Id))`) — only Executives are selectable |
...
| `department` | Choice | internal name is **lowercase**; source of the `Department` picker options on `Project_List`/`Project_ChangeRequests` via `Distinct(Filter('Employee List', !IsBlank(department.Value)), department.Value)` — employees with no department set are excluded |
